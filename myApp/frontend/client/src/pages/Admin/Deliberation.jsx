...
const Deliberation = () => {
  ...
  useEffect(() => {
    ...
    const loadSessionData = async () => {
      setLoading(true);
      try {
        // Récupérer les étudiants de la session avec leurs notes
        const response = await adminService.getSessionStudents(selectedSession.id);
        const studentsData = Array.isArray(response) ? response : [];

        setStudents(studentsData);

        // Préparer l'objet des notes
        const gradesObj = {};
        ...
        if (selectedSession?.id) {
          try {
            ...
            if (deliberationResponse?.data) {
              ...
              // Mettre à jour les étudiants avec les moyennes sauvegardées
            ...
            console.error('Erreur lors du chargement des délibérations:', error);
          }
        }

        // Vérifier l'état de la délibération
        if (selectedSession.deliberation_status) {
          setDeliberationStatus(selectedSession.deliberation_status);
        }
      } catch (error) {
